...
def cas_ratio(box, cluster):
    ...
    ratios = np.concatenate([ratios_of_box_cluster, ratios_of_cluster_box],
                            axis=-1)
...
def avg_ratio(box, cluster):
    return np.mean(
        [np.min(cas_ratio(box[i], cluster)) for i in range(box.shape[0])])
...
def kmeans(box, k):
    ...
    while True:
        ...
        last_clu = near
        if iter % 5 == 0:
            print('iter: {:d}. avg_ratio:{:.2f}'.format(
                iter, avg_ratio(box, cluster)))
        iter += 1
