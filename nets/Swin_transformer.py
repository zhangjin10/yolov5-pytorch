...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        # 3136
        self.num_patches = self.patches_resolution[
            0] * self.patches_resolution[1]
        # 3
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.shift_size > 0:
            ...
            _H, _W = _make_divisible(H, self.window_size), _make_divisible(
                W, self.window_size),
    ...
    def forward(self, x):
        ...
        if type(self.attn_mask) != type(None):
            attn_mask = torch.tensor(
                self.attn_mask).cuda() if x.is_cuda else torch.tensor(
                    self.attn_mask)
        else:
            attn_mask = None
        ...
        attn_windows = attn_windows.view(-1, self.window_size,
                                         self.window_size, C)
        shifted_x = window_reverse(attn_windows, self.window_size, _H,
                                   _W)  # B H' W' C
