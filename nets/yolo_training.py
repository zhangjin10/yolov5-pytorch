...
class YOLOLoss(nn.Module):
    ...
    def BCELoss(self, pred, target):
        ...
        output = -target * torch.log(pred) - (1.0 - target) * torch.log(1.0 -
                                                                        pred)
        return output
    ...
    def forward(self, l, input, targets=None, y_true=None):
        ...
        # -----------------------------------------------#
        prediction = input.view(bs, len(self.anchors_mask[l]), self.bbox_attrs,
                                in_h, in_w).permute(0, 1, 3, 4,
                                                    2).contiguous()
        ...
        #   作为负样本不合适
        # ----------------------------------------------------------------#
        pred_boxes = self.get_pred_boxes(l, x, y, h, w, targets,
    ...
    def get_target(self, l, targets, anchors, in_h, in_w):
        ...
        for b in range(bs):
            ...
            # ------------------------------------------------------------------------------#
            ratios_of_gt_anchors = torch.unsqueeze(
                batch_target[:, 2:4], 1) / torch.unsqueeze(
                    torch.FloatTensor(anchors), 0)
            ratios_of_anchors_gt = torch.unsqueeze(
                torch.FloatTensor(anchors), 0) / torch.unsqueeze(
                    batch_target[:, 2:4], 1)
            ratios = torch.cat([ratios_of_gt_anchors, ratios_of_anchors_gt],
                               dim=-1)
            ...
            for t, ratio in enumerate(max_ratios):
                ...
                for k, mask in enumerate(self.anchors_mask[l]):
                    ...
                    offsets = self.get_near_points(batch_target[t, 0],
                                                   batch_target[t, 1], i, j)
    ...
    def get_pred_boxes(self, l, x, y, h, w, targets, scaled_anchors, in_h,
                       in_w):
        # -----------------------------------------------------#
        #   计算一共有多少张图片
        # -----------------------------------------------------#
        ...
        anchor_w = torch.Tensor(scaled_anchors_l).index_select(
            1, torch.LongTensor([0])).type_as(x)
        anchor_h = torch.Tensor(scaled_anchors_l).index_select(
            1, torch.LongTensor([1])).type_as(x)
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        self.decay = lambda x: decay * (1 - math.exp(
            -x / tau))  # decay exponential ramp (to help early epochs)
    ...
    def yolox_warm_cos_lr(lr, min_lr, total_iters, warmup_total_iters,
        ...
        if iters <= warmup_total_iters:
            ...
            lr = (lr - warmup_lr_start) * pow(
                iters / float(warmup_total_iters), 2) + warmup_lr_start
